...
class EbayOAuthService
{
    public function buildAuthorizationUrl(Company $company): string
    {
        ...
        return $this->authUrl($app) . '?' . http_build_query($params);
    }
    ...
    public function handleCallback(string $code, string $state): SalesChannel
    {
        ...
        $channel = SalesChannel::create([
            ...
            'type' => SalesChannel::TYPE_EBAY,
            'name' => 'eBay ' . now()->format('Y-m-d H:i'),
            'base_url' => null,
            'status' => 'active',
            'settings' => [
                ...
                'expires_at' => now()->addSeconds((int) ($token['expires_in'] ?? 7200))->toIso8601String(),
            ],
    ...
    public function refreshToken(SalesChannel $channel): SalesChannel
    {
        ...
        if (!$refreshToken) {
            throw new \RuntimeException('Brak refresh_token dla kanału eBay. Połącz konto ponownie.');
        }
        ...
        $settings['expires_at'] = now()->addSeconds((int) ($token['expires_in'] ?? 7200))->toIso8601String();
    ...
    {
        $app = MarketplaceAppCredential::query()->forCompanyMarketplace($companyId, MarketplaceAppCredential::MARKETPLACE_EBAY)->first();

        if (!$app) {
            throw new \RuntimeException('Najpierw uzupełnij dane aplikacji eBay w panelu: Ustawienia marketplace.');
        }
